<template>
    <v-container>
        <h2 style="visibility: hidden; fontSize: 0">Merchandise detail</h2>
        <v-row>
            ...
            <v-col cols="6" sm="4" md="2" order-md="3">
                <v-row justify="end" justify-md="start" class="px-4">
                    <v-btn aria-label="save" :disabled="save" depressed color="primary" @click="handleAddMerch" :loading="loadingAddMerch">save</v-btn>
                </v-row>
            </v-col>
                    ...
                    <v-form @submit.prevent ref="form" class="px-4 pt-2 px-md-5 pt-md-3 pb-5">
                        ...
                        <draggable :list="images" class="draggable-grid" draggable=".item" @change="checkIfUpdated">
                            <v-col v-for="(image, i) in imagePreviews" :key="i" style="maxWidth: 120px; minWidth: 120px; maxHeight: 120px; minHeight: 120px" class="item">
                                ...
                                <v-btn aria-label="clear" fab x-small style="marginTop: -235px; marginLeft: 80px" @click="handleRemoveImage(i)"><v-icon>clear</v-icon></v-btn>
                            </v-col>
                        ...
                        <v-row class="mx-0" align="center" v-for="(option, i) in options" :key="i">
                            ...
                            <v-col class="quantity py-0">
                                <v-btn aria-label="decrement" color="primary" depressed fab x-small @click="decrementOptionStock(i)"><v-icon small>remove</v-icon></v-btn>
                                ...
                                <v-btn aria-label="increment" color="primary" depressed fab x-small @click="incrementOptionStock(i)"><v-icon small>add</v-icon></v-btn>
                            </v-col>
                            <v-col class="py-0">
                                <v-btn aria-label="remove" depressed class="primary" @click="handleRemoveOption(i)">Remove</v-btn>
                            </v-col>
                        </v-row>
                        ...
                        <v-row class="mx-0" align="center">
                            ...
                            <v-col class="quantity py-0">
                                <v-btn aria-label="decrement" color="primary" depressed fab x-small @click="decrementNewOptionStock"><v-icon small>remove</v-icon></v-btn>
                                <v-text-field class="px-2 text-center" style="maxWidth: 60px" label="Qty" type="number" min="0" v-model="optionQty"></v-text-field>
                                <v-btn aria-label="increment" color="primary" depressed fab x-small @click="incrementNewOptionStock"><v-icon small>add</v-icon></v-btn>
                            </v-col>
                            <v-col class="py-0">
                                <v-btn aria-label="add" depressed class="primary" @click="handleAddOption">Add</v-btn>
                            </v-col>
                        </v-row>
                        ...
                        <v-row align="center" class="ml-1 mt-6" v-if="options.length < 1">
                            <v-btn aria-label="decrement" color="primary" depressed @click="decrementStock" fab small><v-icon small>remove</v-icon></v-btn>
                            ...
                            <v-btn aria-label="increment" color="primary" depressed @click="incrementStock" fab small><v-icon small>add</v-icon></v-btn>
                        </v-row>
                    </v-form>
                </v-card>
            </v-col>
